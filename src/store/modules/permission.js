...
import { fullPathRoutes } from '@/utils'
...
const mutations = {
  ...
  SET_FULLROUTES: (state, routes) => {
    let fullPath = constantRoutes.concat(routes);
    state.fullRoutes = fullPathRoutes(fullPath)
  },
  ...
  getAsyncRoutes({ commit }) {
    return new Promise((resolve, reject) => {
      getRoleMenu().then(res => {
        ...
        commit('SET_FULLROUTES', trueRoutes);
        resolve(trueRoutes)
      }).catch(error => {
        reject(error)
      })
