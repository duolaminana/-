<template>
  <div class="app-container todo-list">
    <div class="search-box">
      <el-form label-width="115px" inline size="small">
        <el-form-item :label="$t('approvalRemind.approval.businessName')">
          <el-input v-model.trim="postData.businessName" clearable :placeholder="$t('approvalRemind.approval.businessName')" style="width: 200px" />
        </el-form-item>
        <el-form-item :label="$t('approvalRemind.approval.businessType')">
          <el-select v-model="postData.businessType" clearable :placeholder="$t('approvalRemind.approval.businessType')" style="width: 200px">
        ...
        <el-form-item :label="$t('approvalRemind.approval.processStatus')">
          <el-select v-model="postData.processStatus" clearable :placeholder="$t('approvalRemind.approval.processStatus')" style="width: 200px">
            <el-option :label="$t('approvalRemind.approval.processStatusNo')" :value="1"></el-option>
            <el-option :label="$t('approvalRemind.approval.processStatusYes')" :value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-table
      ...
      <el-table-column width="160" :label="$t('approvalRemind.approval.businessType')" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.businessType | businessName(approvalTypeList,isZh)}}
        </template>
      </el-table-column>

      <el-table-column width="150" :label="$t('approvalRemind.approval.businessBid')" prop="businessBid" show-overflow-tooltip/>

      <el-table-column :label="$t('approvalRemind.approval.businessName')" prop="businessName" show-overflow-tooltip/>

      <el-table-column align="center" width="160" :label="$t('approvalRemind.approval.createTime')" prop="createTime">
        <template slot-scope="scope">
          {{scope.row.createTime | timeFmt}}
        </template>
      </el-table-column>

      <el-table-column align="center" width="90" :label="$t('common.table.status')" prop="roleStatus">
        <template slot-scope="scope">
      ...
      <el-table-column header-align="center" :label="$t('common.table.actions')" :width="isZh?'120px': '160px'" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" class="export" size="mini" @click="goDetail(row)">{{$t('common.table.view')}}</el-button>
          <el-button type="primary" v-if="row.processStatus === 1" size="mini" @click="goRouter(row)">{{$t('common.buttonText.dealWith')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
