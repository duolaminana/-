<template>
  <div class="app-container check">
    <div class="edit-title">{{$t('order.sale.transferCheck')}}</div>
    <el-form ref="detailForm" :model="detailForm" label-width="140px" size="small" class="edit-form">
      ...
      <el-form-item style="text-align: center;margin-top: 20px">
        <el-button type="primary" size="small" :loading="confirmLoading" @click="saveClick('checkForm')">{{$t('common.buttonText.submit')}}</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      checkFormRules: {
        adjustAmount: [{ required: true, message: this.isZh ? '请填写调整金额' : 'Fill in the adjust amount', trigger: 'blur' }],
        orderRemarks: [{ required: true, message: this.isZh ? '请填写备注' : 'Fill in the remark', trigger: 'blur' }]
      },
      orderSkuList: [],
      allNodeList: [],
      recordList: [],
  ...
  methods: {
    ...
    changeNumber(item) {
      ...
      this.checkForm.adjustAmount = Math.round((item.productAmount - item.receivedAmount) * 10000) / 10000
    },
    saveClick(formName) {
    ...
    getSaleOrderDetail() {
      saleOrderDetail({ orderBid: this.$route.query.id }).then(res => {
        ...
        this.orderSkuList = this.detailForm.transitradeOrderProductsList.map(e => {
