<template>
  <div class="app-container check">
    <div class="edit-title">{{Number($route.query.saleType) === 1 ? $t('order.sale.transferCheck') :$t('order.sale.orderCheck')}}</div>
        ...
        <el-form ref="checkForm" :model="checkForm" label-width="180px" size="small" :rules="checkFormRules">
          <el-col :span="12">
            <el-form-item :label="$t('order.sale.currency')">
              {{detailForm.orderCurrency | currencyFmt(currencyCodeList)}}
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  filters: {
    ...
    currencyFmt(val, arr) {
      let objArr = arr.filter(e => e.currencyCode === val);
      if (objArr.length) {
        return objArr[0].searchCurrencyCodeStr
      } else {
        return val
      }
    }
