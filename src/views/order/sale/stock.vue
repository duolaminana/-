<template>
  <div class="app-container stock">
    <div class="edit-title">{{Number($route.query.saleType) === 1 ? $t('order.sale.transferStock') : $t('order.sale.orderStock')}}</div>
    <el-form ref="detailForm" :model="detailForm" label-width="150px" size="small" class="edit-form">
      ...
      <el-row class="row-item">
        <div class="row-label" style="width: 100%">{{$t('order.sale.goodsStock')}}：</div>
        <el-form ref="stockForm" :model="stockForm" label-width="140px" size="small" :rules="stockFormRules">
          <el-form-item :label="$t('order.sale.stockCompletion')" prop="completeDegree" :label-width="isZh? '':'300px'">
            <el-input-number v-model="stockForm.completeDegree" controls-position="right" :min="0" :max="100" size="small" style="width: 200px"/> &nbsp;%
          </el-form-item>
          <el-form-item :label="$t('order.sale.stockDate')" prop="attachmentUploadTime">
            <el-date-picker v-model="stockForm.attachmentUploadTime" type="datetime" placeholder="select date" style="width: 200px"/>
          </el-form-item>
          <el-form-item :label="$t('order.sale.stockRemark')" prop="eventRemark">
      ...
      <el-row class="row-item">
        <el-col>
          <div class="topBtns">
            <div class="row-label">{{$t('order.sale.stockRecord')}}：</div>
          </div>
          <el-table
            :data="recordList"
            border
            ...
            <el-table-column align="center" width="170" :label="$t('order.sale.stockDate')" prop="attachmentUploadTime">
              <template slot-scope="scope">
                {{scope.row.attachmentUploadTime | timeFmt}}
              </template>
            </el-table-column>

            <el-table-column align="center" :label="$t('order.sale.completePercent')" prop="completeDegree"/>

            <el-table-column align="center" :label="$t('order.sale.stockRemark')" prop="eventRemark"/>

            <el-table-column align="center" :label="$t('order.sale.attachment')" prop="attachmentName" >
              <template slot-scope="scope">
                <a href="javascript:void (0)" @click="downFile(scope.row.attachmentPath)">{{scope.row.attachmentName}}</a>
              </template>
            </el-table-column>

            <el-table-column align="center" :label="$t('common.table.actions')" width="130" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button type="primary" size="mini" @click="downFile(scope.row.attachmentPath)">{{$t('common.download')}}</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      stockFormRules: {
        attachmentUploadTime: [{ required: true, message: 'Please select stock Date', trigger: 'blur' }],
