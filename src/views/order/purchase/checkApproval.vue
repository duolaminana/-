<template>
  <div class="app-container check">
    <div class="edit-title">{{$t('order.purchase.orderCheckApproval')}}</div>
    <el-form ref="detailForm" :model="detailForm" label-width="140px" size="small" class="edit-form">
      ...
      <el-row class="row-item">
        <el-form ref="checkForm" :model="checkForm" label-width="180px" size="small" :rules="checkFormRules">
          ...
          <el-form-item :label="$t('order.tip.uploadFiles')" style="width: 500px">
            <el-upload
              ref="upload"
...
export default {
  ...
  data() {
    return {
      ...
      checkFormRules: {
        adjustAmount: [{ required: true, message: this.$t('order.tip.adjuestAmount'), trigger: 'blur' }],
        orderRemarks: [{ required: true, message: this.$t('order.tip.remark'), trigger: 'blur' }]
      },
      orderSkuList: [],
      allNodeList: [],
      recordList: [],
  ...
  methods: {
    ...
    beforeRemove(file, fileList) {
      return this.$confirm(this.$t('order.tip.confirmDel')`${ file.name }？`);
    },
