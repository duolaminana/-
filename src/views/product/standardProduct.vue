<template>
  <div class="app-container standardProduct">
    ...
    <el-table
      ...
      <el-table-column align="center" width="130" :label="$t('common.table.updateTime')" sortable prop="updateTime" show-overflow-tooltip>
        <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="$t('product.product.addstandard')" width="800px" center>
      <el-form :model="dialogForm" label-width="120px" label-position="left" inline size="mini" >
        <el-form-item :label="$t('product.categoryTemplate.category')">
          <el-cascader
            :key="isZh"
            style="width: 200px"
...
import { getProductStandardList,
  ...
  exportExcelStandardProductList
} from '@/api/product';
...
export default {
  ...
  methods: {
    ...
    expandTable(row, rows) {
      ...
      if (expandFlag) {
        searchStandardSkuList({ productBId: row.productBId, isShelf: this.postData.isShelf }).then(res => {
    ...
    exportStandardProduct() {
      ...
      exportExcelStandardProductList(obj).then(res => {
        const link = document.createElement('a')
        let blob = new Blob([res.data], { type: 'application/vnd.ms-excel' });
        link.style.display = 'none'
        link.href = URL.createObjectURL(blob);
        if (this.isZh) {
          link.setAttribute('download', '标准产品列表.xlsx')
        } else {
          link.setAttribute('download', 'Standard product list.xlsx')
        }
