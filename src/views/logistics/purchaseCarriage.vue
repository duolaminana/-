<template>
  <div class="app-container stock">
    <div class="edit-title">{{$t('logistics.carriage.purchaseCarriageTitle')}}</div>
...
import { updatePurchaseOrderFlowInfo, selectOrderWorkflowNodeBIdApi } from '@/api/order'
import { getDetailPOFlowById, typeNodePurchaseOrder } from '@/api/purchaseOrder'
...
export default {
  ...
  created() {
    this.getSaleOrderDetail(); // 采购订单详情
    this.getTypeNodePurchaseOrder(); // 根据节点类型获取流程节点
    this.getLogisticsOrderDetailListFun(); // 根据采购订单bid获取物流订单详情列表数据
  },
  filters: {
    timeFmt(val) {
      if (val) {
  ...
  methods: {
    ...
    addLogistic() {
      this.logisticsOrderDetailList.push({
        ...
        orderBId: this.$route.query.id,
        logisticeType: 1,
        orderNo: this.detailForm.purchaseNo,
        orderType: 2,
        totalAmount: this.detailForm.totalAmount,
        customerBId: this.detailForm.sysCompanyBId,
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let obj1 = {
            ...
            orderBId: this.$route.query.id,
            workflowNodeBId: this.$route.query.workflowId
          };
          updatePurchaseOrderFlowInfo(obj1).then(res => {
    ...
    getTypeNodePurchaseOrder() {
      typeNodePurchaseOrder({ orderBid: this.$route.query.id, nodeType: this.$route.query.type }).then(res => {
        this.allNodeList = res.data.wfplist.map((e, index) => {
    ...
    // 根据采购订单bid获取物流订单详情列表数据
    getLogisticsOrderDetailListFun() {
      // 根据采购订单bid和订单节点bid获取某个节点详情
