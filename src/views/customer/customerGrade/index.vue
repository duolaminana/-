    ...
    <el-table
      ...
      <el-table-column label="客户等级图标" width="100px" property="smallMark" >
        <template slot-scope="scope">
          <img v-if="scope.row.smallMark" :src="fullUrl(scope.row.smallMark)" style="border-style: none;width: 70px;height: 70px;">
        </template>
      </el-table-column>
      ...
      <el-form ref="addDepartment" :model="gradeData" label-width="110px" label-position="right" size="small" :rules="gradeDataRules">
        ...
        <el-form-item label="客户等级图标" prop="smallMark">
          <el-upload
            ...
            disabled>
            <img v-if="gradeData.smallMark" :src="fullUrl(gradeData.smallMark)" class="avatar">
        ...
        <el-form-item label="客户等级底图" prop="underlayPath">
          <el-upload
            ...
            disabled>
            <img v-if="gradeData.underlayPath" :src="fullUrl(gradeData.underlayPath)" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
          <div class="el-upload__tip">请上传JPG或PNG格式并大小不超过2MB的照片</div>
        </el-form-item>
