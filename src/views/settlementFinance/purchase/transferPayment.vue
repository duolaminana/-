  ...
  <div class="app-container editPayRecord">
    ...
    <el-form ref="detailForm" :model="detailForm" label-width="140px" size="small" class="edit-form">
      <el-row class="row-item">
        <el-col :span="12">
          <el-form-item :label="$t('settlementFinance.purchase.transferOrderNo')">
            {{detailForm.transitradeOrderDetailVo.orderNo}}
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('settlementFinance.purchase.orderDate')" label-width="80">
            {{detailForm.transitradeOrderDetailVo.createTime | timeFmt}}
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="11">
          ...
          <table class="form-table" border="0" cellspacing="0" cellpadding="0" width="100%">
            <tr>
              ...
              <td width="60%">
                {{detailForm.transitradeOrderDetailVo.sellerName}}
              </td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.shortName')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerShort}}</td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.contactName')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerContact}}</td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.contactNo')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerTelephone}}</td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.address')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerAddress}}</td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.country')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerCountryCode}}</td>
            </tr>
            <!-- <tr>
        ...
        <el-col :span="11">
          ...
          <table class="form-table" border="0" cellspacing="0" cellpadding="0" width="100%">
            ...
            <tr>
              ...
              <td width="60%">
                {{detailForm.transitradeOrderDetailVo.sellerBank}}
              </td>
            </tr>
            <!-- <tr>
            ...
            <!-- <tr>
              <td>银行名称</td><td>{{detailForm.transitradeOrderDetailVo.sellerBank}}</td>
            </tr> -->
            <!-- <tr>
              <td>支行名称</td><td>{{detailForm.transitradeOrderDetailVo.sellerBranchName}}</td>
            </tr> -->
            <!-- <tr>
              <td>支行地址</td><td>{{detailForm.transitradeOrderDetailVo.sellerBankAddress}}</td>
            </tr> -->
            <tr>
              <td>{{$t('settlementFinance.purchase.accountName')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerBankAccount}}</td>
            </tr>
            <tr>
              <td>{{$t('settlementFinance.purchase.swiftCode')}}</td><td>{{detailForm.transitradeOrderDetailVo.sellerSwifcode}}</td>
            </tr>
            <!-- <tr>
      ...
      <el-row class="row-item">
        ...
        <el-col :span="12">
          <el-form-item :label="$t('settlementFinance.purchase.orderAmount')">
            {{detailForm.transitradeOrderDetailVo.totalAmount | moneyFmt}}
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('settlementFinance.purchase.orderCurrency')">
            {{detailForm.transitradeOrderDetailVo.orderCurrency}}
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    savePayRecord(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.addForm.paymentOrderType = 3;
          this.addForm.paymentInstructNo = this.$route.query.paymentInstructNo;
          this.addForm.paymentAssociatedOrders = this.detailForm.transitradeOrderDetailVo.paymentAssociatedOrders;
          this.addForm.associatedOrderBId = this.$route.query.id;
          this.addForm.workflowProcessBId = this.$route.query.workflowId;
          this.addForm.customerBId = this.detailForm.transitradeOrderDetailVo.supplierBId;
    ...
    // 根据节点类型获取流程节点
    getSaleOrderDetail() {
      detailPaymentDataApi({ bid: this.$route.query.id, type: 3 }).then(res => {
        this.detailForm = Object.assign(this.detailForm, res.data);
        this.addForm.paymentCurrency = this.detailForm.transitradeOrderDetailVo.orderCurrency;
        this.recRecordList = (res.data.paymentRecordVoList || []).map(e => {
          e.fileName = e.paymentDocList.map(e => e.attachmentName).join(',');
          return { ...e }
        });
        this.getCustomerBankList(this.detailForm.transitradeOrderDetailVo.customerBId, this.detailForm.buyerBankId);
        this.getCompanyBankList(this.detailForm.transitradeOrderDetailVo.sysCompanyBId, this.detailForm.sellerBankId);
      }).catch(err => {
        this.$message.error(err.message)
      })
    }
