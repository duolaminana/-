<template>
  <div class="app-container menu-page">
    <div class="topBtns">
      ...
      <el-button type="primary" size="small" class="export" @click="outportData">导出excel</el-button>
    </div>
    <el-table
      ref="listTable"
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?$t('system.menu.addMenu'):$t('system.menu.editMenu')" width="500px" center :close-on-click-modal="false" @closed="clearData">
      <el-form ref="addClientMenu" v-if="dialogVisible" :model="menuData" label-width="145px" label-position="left" size="small" :rules="menuDataRules">
        <el-form-item :label="$t('system.menu.parentMenu')">
          <el-cascader
            class="form-dialog-width"
            v-model="menuData.parentsName"
            :options="disabledList"
            :props="{value: 'bid', label: isZh?'menuTitleZh':'menuTitle', children: 'children'}"
            filterable
            clearable
            placeholder="search"
            change-on-select
...
<script>
import { getClientMenuList, getClientMenuTwoList, addClientMenu, changeClientMenuStatus, editClientMenu, deleteClientMenu, getMenuClientRole, clientMenuMoveOne, clientMenuMoveMast, exportFrontendMenuList } from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    getDataTwoList() {
      getClientMenuTwoList().then(res => {
        this.dataTwoList = res.data.map(e => {
          return { ...e }
        })
      }).catch(err => {
        this.$message.error(err.message)
    ...
    deleteClientMenu() {
      ...
      if (this.selectList.length > 1) {
        return this.$message.warning(this.$t("system.userManage.only"));
      }
    ...
    outportData() {
      let obj = {};
      exportFrontendMenuList(obj).then(res => {
        const link = document.createElement('a');
        let blob = new Blob([res.data], { type: 'application/vnd.ms-excel' });
        link.style.display = 'none';
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', 'frontendMenuList.xls');
