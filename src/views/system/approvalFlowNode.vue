<template>
  <div class="app-container roleManagement">
    <div class="search-box">
      <el-form label-width="180px" inline size="small">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?$t('system.approvalFlow.addFlowNode'):$t('system.approvalFlow.editFlowNode')" center width="600px">
      <el-form :model="entity" ref="ruleForm" v-if="dialogVisible" label-width="155px" label-position="right" size="small" :rules="entityDataRules">
        ...
        <el-form-item :label="$t('system.approvalFlow.isLogic')" prop="isLogic">
          <el-select v-model="entity.isLogic" placeholder="treatment conditions" disabled style="width: 300px">
        ...
        <el-form-item :label="$t('system.approvalFlow.nodeStatus')" prop="nodeStatus">
          <el-select v-model="entity.nodeStatus" disabled clearable placeholder="node status" style="width: 300px">
...
export default {
  ...
  data() {
    return {
      ...
      entityDataRules: {
        nodeCode: [
          { required: true, message: this.$t('system.notice.nodeCode'), trigger: 'blur' }
        ],
        nodeName: [
          { required: true, message: this.$t('system.notice.nodeName'), trigger: 'blur' }
        ],
        isLogic: [
          { required: true, message: this.$t('system.notice.isLogic'), trigger: 'change' }
        ],
        disposeUserList: [
          { required: true, message: this.$t('system.notice.disposeUserList'), trigger: 'change' }
        ],
        nodeLevel: [
          { required: true, message: this.$t('system.notice.nodeLevel'), trigger: 'blur' }
        ],
        nodeStatus: [
          { required: true, message: this.$t('system.notice.nodeStatus'), trigger: 'change' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    openDialog(type, row) {
      ...
      if (type === 'edit') {
        ...
        this.entity.isLogic = 1;
        this.entity.nodeStatus = 1;
