<template>
  <div class="app-container roleManagement">
      ...
      <el-form :label-width="isZh ? '100px' : '100px'" inline size="small">
        <!--<div>-->
          <el-form-item :label="$t('system.approvalFlow.name')">
            <el-input v-model.trim="postData.approvalName" clearable :placeholder="$t('system.approvalFlow.name')" style="width: 200px"/>
          </el-form-item>
          <el-form-item :label="$t('system.approvalFlow.code')">
            <el-input v-model.trim="postData.approvalCode" clearable :placeholder="$t('system.approvalFlow.code')" style="width: 200px"/>
          </el-form-item>
        <!--</div>-->
        <el-form-item :label="$t('system.approvalFlow.status')">
          <el-select v-model="postData.approvalStatus" clearable :placeholder="$t('system.approvalFlow.status')" style="width: 200px">
            <el-option v-for="(item, index) in statusList2" :label="language === 'zh' ? item.labelZh : item.label" :value="item.id" :key="index"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-table
      ...
      <el-table-column header-align="center" :label="$t('common.table.actions')" :width="isZh?180:210" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" class="export" @click="clickMethod(row.bid,row.approvalCode,row.approvalName)">{{$t('system.approvalFlow.editNode')}}</el-button>
          <!--<el-button type="primary" size="mini" @click="openDialog('edit', row)">{{$t('common.table.edit')}}</el-button>-->
          <el-button :type="row.approvalStatus === 1 ? 'info' : 'danger'" size="mini" class="table-btn-width" @click="changeStatus(row)">
            {{row.approvalStatus === 1 ? $t('common.table.disable') : $t('common.table.enable')}}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?$t('system.approvalFlow.addFlow'):$t('system.approvalFlow.editFlow')"
               center width="650px">
      <el-form :model="entity" v-if="dialogVisible" ref="ruleForm" label-width="180px" label-position="right" size="small" :rules="entityDataRules">
        ...
        <el-form-item :label="$t('system.approvalFlow.module')">
          <el-select v-model="entity.approvalType" clearable placeholder="Business module">
            <el-option v-for="(item, index) in approvalTypeList" :label="isZh?item.label:item.labelEn" :value="item.value" :key="index"></el-option>
          </el-select>
...
export default {
  ...
  data() {
    return {
      ...
      entityDataRules: {
        approvalCode: [
          { required: true, message: this.$t('system.notice.approvalCode'), trigger: 'blur' }
        ],
        approvalName: [
          { required: true, message: this.$t('system.notice.approvalName'), trigger: 'blur' }
        ],
        ownModule: [
          { required: true, message: 'Please select notice business module', trigger: 'change' }
        ],
        approvalStatus: [
          { required: true, message: this.$t('system.notice.approvalStatus'), trigger: 'change' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    deleteBatchMethod() {
      ...
      this.$confirm(this.$t("system.userManage.whether"), this.$t("system.userManage.hint"), {
        ...
        approvalFlowDeleteByIds(formData).then(res => {
          if (res.status === 200) {
            this.$message.success(this.$t('system.userManage.delete'));
          }
          this.getDataList();
        }).catch(err => {
          this.$message.error(err.message);
    ...
    outportData() {
      ...
      exportByApprovalFlowList(obj).then(res => {
        const link = document.createElement('a');
        let blob = new Blob([res.data], { type: 'application/vnd.ms-excel' });
        link.style.display = 'none';
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', 'approvalFlowList.xls');
