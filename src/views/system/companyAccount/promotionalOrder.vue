      ...
      <el-row :gutter="28">
        <el-col>
          <el-form-item :label="$t('system.business.currencyCode')" prop="currencyCode2">
            <el-select filterable remote :remote-method="selectCurrencyCodeListFun4" :loading="loading1" v-model="formMaterial.currencyCode2" clearable :placeholder="$t('system.business.searchCurrencyCode')" style="width: 250px">
...
export default {
  ...
  created() {
    this.selectSysOrderParameterFun4(); // 根据类型获取数据
    this.selectgetDictionaryBykeyFun4(); // 数据字典， 获取允许附件列表值
    this.selectgetDictionaryPriceGradientFun4(); // 数据字典， 获取梯度值
    this.selectgetDictionaryVisiblePriceFun4(); // 数据字典，获取默认显示价格值
    this.selectCurrencyCodeListFun4(); // 币种列表
  },
  methods: {
    selectSysOrderParameterFun4() {
      selectSysOrderParameterApi({ orderParameterType: 4 }).then(res => {
    ...
    selectCurrencyCodeListFun4(val) {
      const _this = this;
      if (val !== '') {
        this.loading1 = true;
    ...
    // 数据字典，获取梯度值
    selectgetDictionaryPriceGradientFun4() {
    ...
    // 数据字典，获取默认显示价格值
    selectgetDictionaryVisiblePriceFun4() {
      let data = {};
    ...
    onSubmit(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.formMaterial.orderParameterType = 4;
          ...
          updateSysOrderParameterApi(this.formMaterial).then(res => {
            ...
            this.selectSysOrderParameterFun4();
          }).catch(err => {
