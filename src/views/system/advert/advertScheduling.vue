...
<template>
  <div class="app-container form-test">
      ...
      <el-form :label-width="isZh ? '100px' : '130px'" inline size="small">
        ...
        <div>
          <el-form-item :label="$t('system.ad.beginTime')" prop="reBeginTime">
            ...
            type="datetime"
            style="width: 200px"
            :placeholder="$t('system.ad.beginTime')">
            </el-date-picker>
          </el-form-item>
          <el-form-item :label="$t('system.ad.endTime')" prop="reBeginTime">
            <el-date-picker
              ...
              type="datetime"
              style="width: 200px"
              :placeholder="$t('system.ad.endTime')">
            </el-date-picker>
          </el-form-item>
          <el-form-item :label="$t('system.ad.adSequence')">
            <el-input v-model.trim="postData.adSequence" clearable :placeholder="$t('system.ad.adSequence')" style="width: 200px" />
          </el-form-item>
          <el-form-item>
    ...
    <el-table
      ...
      <el-table-column align="center" :label="$t('system.ad.beginTime')" width="150px" prop="startDate" show-overflow-tooltip>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='add'?$t('system.ad.addScheduling'): $t('system.ad.editScheduling')"
      ...
      <el-form ref="addCompany" v-if="dialogVisible" :model="menuData" label-width="165px" label-position="left" size="small" :rules="menuDataRules">
        ...
        <el-form-item :label="$t('system.ad.adSequence')" prop="adSequence">
          <el-input-number :max="15" :min="1" :step="1" :precision="0" v-model="menuData.adSequence" controls-position="right" clearable :placeholder="$t('system.ad.adSequence')" style="width: 300px" class="form-dialog-width" />
        </el-form-item>
        <el-form-item :label="$t('system.ad.AdvertisementContent')" prop="sysAdDetailBId">
    ...
    <el-dialog :visible.sync="detailModal" :title="$t('system.ad.schedulingDetail')" center @closed="clearData" width="600px">
      <el-form ref="addCompany" :model="menuData" label-width="165px" label-position="left" size="small" :rules="menuDataRules">
        <el-form-item :label="$t('system.ad.positionName')" prop="adPositionBId">
          <el-select v-model="menuData.adPositionBId" clearable
                     :placeholder="$t('system.ad.positionName')"
                     style="width: 300px"
...
export default {
  ...
  data() {
    return {
      ...
      menuDataRules: {
        sysAdDetailBId: [
          { required: true, message: this.$t('system.ad.adsysAdDetailBIdHint'), trigger: 'change' }
        ],
        adPositionBId: [
          { required: true, message: this.$t('system.ad.positionBIdHint'), trigger: 'change' }
        ],
        intermediary: [
          { required: true, message: this.$t('system.ad.intermediaryHint'), trigger: 'change' }
        ],
        adSequence: [
          { required: true, message: this.$t('system.ad.adSequenceHint'), trigger: 'blur' }
        ],
        startDate: [
          { required: true, message: this.$t('system.ad.startDateHint'), trigger: 'change' }
        ],
        endDate: [
          { required: true, message: this.$t('system.ad.endDateHint'), trigger: 'change' }
        ]
      },
      pickerOptionsStart: {
        disabledDate: (time) => {
          if (this.menuData.endDate) {
            return time.getTime() > this.menuData.endDate;
          }
        }
      },
      pickerOptionsEnd: {
        disabledDate: (time) => {
          if (this.menuData.startDate) {
            return time.getTime() < this.menuData.startDate
          }
        }
      },
      postData: {
  ...
  methods: {
    ...
    openDetail(row) {
      ...
      getListApi({ bId: row.adPositionBId }).then(res => {
        this.adTitleList = res.data;
      }).catch(err => {
        this.$message.error(err.message);
      });
    ...
    outportData() {
      let obj = {
        ...
        intermediary: this.postData.intermediary,
        adSequence: this.postData.adSequence,
        startDate: this.postData.startDate,
        endDate: this.postData.endDate,
        adSchedulingStatus: this.postData.adSchedulingStatus
      };
      exportBySysAdSchedulingExcel(obj).then(res => {
        const link = document.createElement('a');
        let blob = new Blob([res.data], { type: 'application/vnd.ms-excel' });
        link.style.display = 'none';
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', 'SysAdScheduling.xls');
