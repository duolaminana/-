      ...
      <el-form :label-width="isZh ? '100px' : '125px'" inline size="small">
        <div>
          <el-form-item :label="$t('system.agreement.agreeName')">
            <el-input v-model.trim="postData.agreementName" clearable style="width: 200px"/>
          </el-form-item>
          <el-form-item :label="$t('system.agreement.undersigned')">
            <el-input v-model.trim="postData.signer" clearable style="width: 200px"/>
          </el-form-item>
          <el-form-item>
...
<!--      <el-table-column :label="$t('system.agreement.fileName')" align="center" prop="agreementName" show-overflow-tooltip/>-->
      <el-table-column align="center" :label="$t('system.agreement.version')" width="130" prop="versionNumber" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column :label="$t('system.agreement.fileName')" align="center" prop="agreementName" show-overflow-tooltip/>
      <el-table-column align="center" :label="$t('system.role.status')" width="180" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" class="export" @click="openDetail(row)">{{$t('system.agreement.view')}}</el-button>
          <el-button type="warning" size="mini" @click="downFile(row.agreementPath)">{{$t('system.agreement.download')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="detailModal3" :title="$t('system.agreement.agreementDetail')" center width="500px">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="danger" size="small" @click="closeDetail">{{$t('common.buttonText.close')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    getDataList() {
      ...
      sysSigningList({
        ...
        total: this.postData.total,
        agreementName: this.postData.agreementName,
        signer: this.postData.signer
      }).then(res => {
