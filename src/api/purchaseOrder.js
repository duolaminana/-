...
// 查询采购订单流程节点-根据类型
export function typeNodePurchaseOrder(option) {
  return request.get(`/backend-order/purchaseOrder/selectWorkFlowTypeByOrderBid`, { params: option });
}
...
export function exportPurchaseOrderExcel(option) {
  return axios({
    method: 'post',
    data: option,
    url: `${uploadBaseApi}/backend-order/purchaseOrder/exportPurchaseOrderExcel`,
    headers: {
      token: getToken()
    },
    responseType: 'blob'
