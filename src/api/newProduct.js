 ...
 * 问卷列表-导出
 */
export function exportEasypoi(option) {
  return axios({
    method: 'post',
    data: option,
    url: `${uploadBaseApi}/backend-product/questionnaire/exportEasypoi`,
    headers: {
      token: getToken()
    },
    responseType: 'blob'
...
export function exportExcelApplySampleList(option) {
  return axios({
    method: 'post',
    data: option,
    url: `${uploadBaseApi}/backend-product/productNewApplyRecord/exportNewRecordExcel`,
    headers: {
      token: getToken()
    },
    responseType: 'blob'
